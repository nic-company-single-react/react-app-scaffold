...
import { Type, ArrowLeft, TriangleAlert, Search, X } from 'lucide-react';
...
/* ────────────────────────────────────────────────────────────
 * $util.string 함수 목록
 * IStringUtil 인터페이스(설명 + @demo 예시값 + 시그니처)에서 완전 자동으로 추출합니다.
 * 단일 출처 = types/common/index.ts. (인터페이스에 함수를 추가하면 데모까지 그대로 반영)
 * ──────────────────────────────────────────────────────────── */
const NS = '$util.string';
const stringFns = parseFunctionDocs(typeSource, 'IStringUtil');

export default function StringUtil(): React.ReactNode {
	...
	/* ── 목록 ↔ 실제 구현 동기화 검사 ───────────────────────────
	 * 파싱된 목록(stringFns)을 실제 $util.string 구현과 대조합니다.
	 * - missing: 구현에는 있지만 목록에 없는 함수 (JSDoc 누락 등 → 추가 필요)
	 * - stale  : 목록에는 있지만 구현에서 사라진 함수 (제거 필요)
	 * ────────────────────────────────────────────────────────── */
	const documented = stringFns.map((f) => f.name);
	const implemented = Object.keys($util.string);
	...
	const filteredFns = useMemo(() => {
		...
		if (!q) return stringFns;
		return stringFns.filter(
			(fn) =>
				...
				<div className="max-w-3xl space-y-8">
					...
					<div className="space-y-3">
						...
						<div className="flex items-center gap-3">
							...
							<div>
								<h1 className="text-2xl font-bold text-gray-900 dark:text-white">string 유틸</h1>
								<p className="text-sm text-gray-500 dark:text-gray-400">
									...
									의 문자열 관련 함수들을 직접 입력하며 실행해 볼 수 있습니다.
								</p>
							</div>
						</div>
					</div>
					...
					<div className="relative">
						...
						<input
							...
							placeholder="함수명 또는 설명으로 검색 (예: 마스킹)"
					...
					{noResult ? (
						...
						filteredFns.map((doc) => (
							<UtilFunctionDemo
								...
								doc={doc}
								ns={NS}
								impl={$util.string as unknown as Record<string, (...args: never[]) => unknown>}
							/>
						))
					)}
				</div>
				...
				<aside className="hidden lg:block">
					<div className="sticky top-24 flex max-h-[calc(100vh-7rem)] flex-col space-y-3">
							...
							<div className="space-y-1.5 rounded-xl border border-amber-300 bg-amber-50 p-3 dark:border-amber-700/60 dark:bg-amber-900/20">
								...
								{missing.length > 0 && (
									<p className="text-[11px] leading-relaxed text-amber-700 dark:text-amber-300/90">
										데모 누락: <code className="font-mono font-semibold">{missing.join(', ')}</code> — $util.string에는
						...
						<p className="px-1 text-[11px] leading-relaxed text-gray-400 dark:text-gray-500">
							{query
								? `검색 결과 ${filteredFns.length} / ${stringFns.length}개`
