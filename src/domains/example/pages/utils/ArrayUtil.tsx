...
import { ListTree, ArrowLeft, TriangleAlert, Search, X } from 'lucide-react';
...
/* ────────────────────────────────────────────────────────────
 * $util.array 함수 목록
 * IArrayUtil 인터페이스(설명 + @demo 예시값 + 시그니처)에서 완전 자동으로 추출합니다.
 * 배열 인자는 JSON 문자열로 입력합니다. (데모 카드가 JSON.parse 합니다)
 * ──────────────────────────────────────────────────────────── */
const NS = '$util.array';
const arrayFns = parseFunctionDocs(typeSource, 'IArrayUtil');

export default function ArrayUtil(): React.ReactNode {
	const scrollToFn = (id: string) => {
		document.getElementById(id)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
	};

	/* ── 목록 ↔ 실제 구현 동기화 검사 ─────────────────────────── */
	const documented = arrayFns.map((f) => f.name);
	const implemented = Object.keys($util.array);
	...
	const filteredFns = useMemo(() => {
		...
		if (!q) return arrayFns;
		return arrayFns.filter(
			(fn) =>
				...
				<div className="max-w-3xl space-y-8">
					<div className="space-y-3">
						...
						<div className="flex items-center gap-3">
							<div className="flex size-10 items-center justify-center rounded-xl bg-indigo-50 dark:bg-indigo-900/20">
								<ListTree className="size-5 text-indigo-600 dark:text-indigo-400" />
							</div>
							<div>
								<h1 className="text-2xl font-bold text-gray-900 dark:text-white">array 유틸</h1>
								<p className="text-sm text-gray-500 dark:text-gray-400">
									...
									의 목록 집계·정렬·트리 변환 함수들을 직접 실행해 볼 수 있습니다. (배열은 JSON으로 입력)
								</p>
							</div>
						</div>
					</div>
					<div className="relative">
						...
						<input
							...
							placeholder="함수명 또는 설명으로 검색 (예: 트리)"
					...
					{noResult ? (
						...
						filteredFns.map((doc) => (
							<UtilFunctionDemo
								...
								doc={doc}
								ns={NS}
								impl={$util.array as unknown as Record<string, (...args: never[]) => unknown>}
							/>
						))
					)}
				</div>
				...
				<aside className="hidden lg:block">
					<div className="sticky top-24 flex max-h-[calc(100vh-7rem)] flex-col space-y-3">
							...
							<div className="space-y-1.5 rounded-xl border border-amber-300 bg-amber-50 p-3 dark:border-amber-700/60 dark:bg-amber-900/20">
								...
								{missing.length > 0 && (
									<p className="text-[11px] leading-relaxed text-amber-700 dark:text-amber-300/90">
										데모 누락: <code className="font-mono font-semibold">{missing.join(', ')}</code> — $util.array에는
						...
						<p className="px-1 text-[11px] leading-relaxed text-gray-400 dark:text-gray-500">
							{query
								? `검색 결과 ${filteredFns.length} / ${arrayFns.length}개`
