...
import { Landmark, ArrowLeft, TriangleAlert, Search, X } from 'lucide-react';
...
/* ────────────────────────────────────────────────────────────
 * $util.finance 함수 목록
 * IFinanceUtil 인터페이스(설명 + @demo 예시값 + 시그니처)에서 완전 자동으로 추출합니다.
 * 단일 출처 = types/common/index.ts. (인터페이스에 함수를 추가하면 데모까지 그대로 반영)
 * ──────────────────────────────────────────────────────────── */
const NS = '$util.finance';
const financeFns = parseFunctionDocs(typeSource, 'IFinanceUtil');

export default function FinanceUtil(): React.ReactNode {
	...
	/* ── 목록 ↔ 실제 구현 동기화 검사 ─────────────────────────── */
	const documented = financeFns.map((f) => f.name);
	const implemented = Object.keys($util.finance);
	...
	const filteredFns = useMemo(() => {
		...
		if (!q) return financeFns;
		return financeFns.filter(
			(fn) =>
				...
				<div className="max-w-3xl space-y-8">
					<div className="space-y-3">
						...
						<div className="flex items-center gap-3">
							<div className="flex size-10 items-center justify-center rounded-xl bg-indigo-50 dark:bg-indigo-900/20">
								<Landmark className="size-5 text-indigo-600 dark:text-indigo-400" />
							</div>
							<div>
								<h1 className="text-2xl font-bold text-gray-900 dark:text-white">finance 유틸</h1>
								<p className="text-sm text-gray-500 dark:text-gray-400">
									...
									의 이자·상환·환율 등 금융 계산 함수들을 직접 입력하며 실행해 볼 수 있습니다.
								</p>
							</div>
						</div>
					</div>
					<div className="relative">
						...
						<input
							...
							placeholder="함수명 또는 설명으로 검색 (예: 이자)"
					...
					{noResult ? (
						...
						filteredFns.map((doc) => (
							<UtilFunctionDemo
								...
								doc={doc}
								ns={NS}
								impl={$util.finance as unknown as Record<string, (...args: never[]) => unknown>}
							/>
						))
					)}
				</div>
				...
				<aside className="hidden lg:block">
					<div className="sticky top-24 flex max-h-[calc(100vh-7rem)] flex-col space-y-3">
							...
							<div className="space-y-1.5 rounded-xl border border-amber-300 bg-amber-50 p-3 dark:border-amber-700/60 dark:bg-amber-900/20">
								...
								{missing.length > 0 && (
									<p className="text-[11px] leading-relaxed text-amber-700 dark:text-amber-300/90">
										데모 누락: <code className="font-mono font-semibold">{missing.join(', ')}</code> — $util.finance에는
						...
						<p className="px-1 text-[11px] leading-relaxed text-gray-400 dark:text-gray-500">
							{query
								? `검색 결과 ${filteredFns.length} / ${financeFns.length}개`
