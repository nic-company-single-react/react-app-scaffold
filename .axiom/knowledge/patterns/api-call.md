...
> ⚠️ **이 문서는 내부 클라이언트 계층(`callApi`·`ApiError`·설정)입니다.** 앱(화면) 개발자는 이걸 직접
> 쓰지 말고 **`useApi` 훅**으로 API 통신을 합니다 — `patterns/use-api.md` 참고. 이 문서는 내부 동작이
> 궁금하거나 클라이언트 설정(`initApiConfig`·인터셉터)을 만질 때만 필요합니다.
