---
title: "useApi 훅 소스 (use-api.ts)"
category: source
tags: [useApi, use-api, 소스, source, 구현, implementation, overload, IUseApiQueryOptions, IUseApiMutationOptions, normalizeBody, FormData]
priority: 1
language: ko
scope: source
...
# 소스: src/core/hooks/use-api.ts

`useApi` 훅 전체 구현. `@axiom/hooks`로 임포트.

TanStack Query의 `useQuery`(조회)와 `useMutation`(변경)을 단일 인터페이스로 통합.
`type` 옵션 또는 `method` 값을 기반으로 어느 쪽 동작인지 자동 결정된다.
